  ...
  if (sorted.length === 0) {
    return (
      <div className="text-center py-10 text-gray-500">
        ...
        <p className="text-sm mt-1 text-gray-600">Tap + to log your first day</p>
      </div>
    );
  }
...
function TradeRow({ trade, onDelete }) {
  ...
  // Parse date safely regardless of timezone
    ...
    <div className="bg-gray-900 rounded-xl px-4 py-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {trade.instrument && (
            <span className="text-xs text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded">{trade.instrument}</span>
          )}
        </div>
        {trade.notes && (
          <p className="text-xs text-gray-400 truncate mt-0.5">{trade.notes}</p>
        )}
      </div>
      <span className={`text-base font-bold flex-shrink-0 ${isProfit ? 'text-green-400' : 'text-red-400'}`}>
      ...
      {confirming ? (
        <div className="flex gap-1 flex-shrink-0">
          <button
            ...
            className="text-xs text-red-400 bg-red-900/40 rounded-lg px-2 py-1.5 leading-none"
          >
            Delete
          </button>
          <button
            onClick={() => setConfirming(false)}
            className="text-xs text-gray-400 bg-gray-800 rounded-lg px-2 py-1.5 leading-none"
          >
            Cancel
          </button>
        </div>
        ...
        <button
          ...
          className="text-gray-700 hover:text-red-400 text-xl leading-none transition-colors flex-shrink-0"
